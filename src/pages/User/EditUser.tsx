...
interface User {
  ...
  email: string;
  password: string;
  role: string; // should hold role_id
  status: string;
  avatar?: string | null;
}
...
export default function EditUser() {
  ...
  console.log("asd", location.state);
  ...
  const { editSuccess } = useAppSelector((state) => state.user);
  ...
  useEffect(() => {
    if (userData) {
      setFormData({
        ...
        password: userData.password || "", // leave empty for security; user may update if needed
        role: userData.role_id || userData.role?.id || "",
