...
export function TenantManagement({
  ...
  const confirmDelete = () => {
    if (deleteDialog.tenant) {
      setTenants((prev) =>
        prev.filter((tenant) => tenant.id !== deleteDialog.tenant!.id)
      );
  ...
  const hasCreatePermission = singleRole?.permissions?.some((perm) => {
    const pageName = perm.page?.name?.toLowerCase().trim();
    return pageName === "tenants" && perm.can_edit;
  });
  if (role === "superAdmin" || hasCreatePermission) {
  }
  return (
